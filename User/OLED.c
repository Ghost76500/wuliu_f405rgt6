...
/***************************************************************************************
  ...
  * 当前程序版本：			v1.3(hal库版本)
  * 当前版本发布时间：		2025.11.12
  * 增加了新功能：			字模库增加了16x64点阵字模，可显示更大字符。
  *                        增加图模库：手写数字0~3及加号图模
  * 
...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	...
	else if (FontSize == OLED_16X64) // 字体为宽16像素，高64像素（大字体）
	{
		/* 将用户在 OLED_Data.c 中提供的 OLED_F16x64 字模显示。每字符占用 128 字节（16 * 8）。
		   数组索引映射为：OLED_F16x64[Char - ' ']（对应从空格开始的可打印 ASCII）。
		*/
		if (Char >= ' ' && Char <= '~')
		{
			OLED_ShowImage(X, Y, 16, 64, OLED_F16x64[Char - ' ']);
		}
		else
		{
			/* 非可打印字符使用空白占位 */
			static const uint8_t blank16x64[128] = {0};
			OLED_ShowImage(X, Y, 16, 64, blank16x64);
		}
	}
}
